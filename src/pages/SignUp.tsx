...
const SignUp = () => {
  ...
  // Google OAuth — both tabs use the same entry. The redirect lands on
  // /signup/complete, which routes existing recruiters by status and
  // walks new ones through the vetting-fields completion step (LinkedIn,
  // company, optional job posting). No public.users row is ever created
  // from the bare session.
  const handleGoogle = async () => {
    setGoogleLoading(true);
    try {
      const { error: oauthErr } = await supabase.auth.signInWithOAuth({
        provider: 'google',
        options: { redirectTo: `${window.location.origin}/signup/complete` },
      });
      if (oauthErr) {
    ...
    <div className="min-h-screen flex">
      ...
      <div className="w-full lg:w-[480px] xl:w-[540px] flex flex-col bg-[#f8f8f8] px-10 py-12 shrink-0">
        ...
        <div className="flex-1 flex flex-col justify-center max-w-sm w-full mx-auto lg:mx-0">
          ...
          {/* Google OAuth — shared by both tabs */}
          <button
            type="button"
